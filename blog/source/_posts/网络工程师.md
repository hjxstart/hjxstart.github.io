...
- 9. 软件的（）是以用户为主，包括软件开发人员和质量保证人员都参加的考试，一般使用实际应用数据进行测试，除了测试软件功能和性能外，还对软件可移植性、兼容性、可维护性、错误的回复功能等进行确认。
...
- 50. Windows系统想要接收并转发本地货远程SNMP代理产生的陷阱消息，则需要开启的服务是（）。
